...
The core flow: every request is identified by a key (default: client IP), the store tracks consumption against the chosen algorithm, and the result includes IETF-compliant rate limit headers.
