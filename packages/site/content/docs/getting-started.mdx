...
Most rate limiters are tied to a single framework or runtime. `universal-rate-limit` is built on the Web Standards `Request`/`Response` API,
so the same core works on Node.js, Bun, Deno, Cloudflare Workers, and Vercel Edge — no rewrites, no adapters to learn from scratch.
...
- **IETF-compliant headers** — draft-6 and draft-7 rate limit headers plus
  [`Retry-After`](https://www.rfc-editor.org/rfc/rfc9110#section-10.2.3) out of the box
...
Complete working apps with integration tests are available for each framework:
